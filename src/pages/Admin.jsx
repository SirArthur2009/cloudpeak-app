      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {puppies.map(p => (
          <div key={p.id} style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '10px', padding: '0.75rem', display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
            {p.photo_url ? <img src={p.photo_url} alt={p.name} style={{ width: '56px', height: '56px', objectFit: 'cover', borderRadius: '8px', flexShrink: 0 }} /> : <div style={{ width: '56px', height: '56px', background: '#f0f0f0', borderRadius: '8px', flexShrink: 0 }} />}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', flexShrink: 0 }}>
              {p.status === 'reserved' && <button onClick={() => handleUnreserve(p)} style={{ ...btnStyle, background: '#fff4e5', color: '#b36200', padding: '0.3rem 0.7rem', fontSize: '0.8rem' }}>Unreserve</button>}
...
function DogsTab() {
  ...
  async function handleSave() {
    setSaving(true)
    setMessage('')
    const payload = { name: form.name, registration_number: form.registration_number, pedigree_url: form.pedigree_url, photo_url: form.photo_url, embark_url: form.embark_url || null, ofa_url: form.ofa_url || null, notes: form.notes }
      ...
      {editing && (
        <div style={{ background: '#f5f5f3', border: '1px solid #e0e0e0', borderRadius: '10px', padding: '1.25rem', marginBottom: '1.5rem' }}>
          ...
          <FormGrid>
            ...
            <div><label style={{ fontSize: '0.8rem', color: '#666' }}>Embark Profile URL</label><input style={inputStyle} value={form.embark_url} onChange={e => setForm({ ...form, embark_url: e.target.value })} placeholder="https://my.embarkvet.com/dog/..." /></div>
            <div><label style={{ fontSize: '0.8rem', color: '#666' }}>OFA Profile URL</label><input style={inputStyle} value={form.ofa_url} onChange={e => setForm({ ...form, ofa_url: e.target.value })} placeholder="https://ofa.org/advanced-search/..." /></div>
            <div style={{ gridColumn: '1 / -1' }}><label style={{ fontSize: '0.8rem', color: '#666' }}>Notes</label><textarea style={{ ...inputStyle, height: '80px', resize: 'vertical' }} value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} /></div>
          </FormGrid>
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
            <button onClick={handleSave} disabled={saving} style={{ ...btnStyle, background: '#1a1a1a', color: '#fff', flex: 1, padding: '0.75rem' }}>{saving ? 'Saving...' : 'Save'}</button>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {dogs.map(d => (
          <div key={d.id} style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '10px', padding: '0.75rem', display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
            {d.photo_url ? <img src={d.photo_url} alt={d.name} style={{ width: '56px', height: '56px', objectFit: 'cover', borderRadius: '8px', flexShrink: 0 }} /> : <div style={{ width: '56px', height: '56px', background: '#f0f0f0', borderRadius: '8px', flexShrink: 0 }} />}
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.4rem', flexWrap: 'wrap' }}>
                {d.pedigree_url && <a href={d.pedigree_url} target="_blank" rel="noreferrer" style={{ fontSize: '0.75rem', color: '#1a1a1a', textDecoration: 'underline' }}>Pedigree</a>}
                {d.embark_url && <a href={d.embark_url} target="_blank" rel="noreferrer" style={{ fontSize: '0.75rem', color: '#5b4fcf', textDecoration: 'underline' }}>Embark</a>}
                {d.ofa_url && <a href={d.ofa_url} target="_blank" rel="noreferrer" style={{ fontSize: '0.75rem', color: '#1a6b3c', textDecoration: 'underline' }}>OFA</a>}
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', flexShrink: 0 }}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', overflowX: 'auto', paddingBottom: '0.25rem', WebkitOverflowScrolling: 'touch' }}>
        {['puppies', 'litters', 'dogs', 'waitlist'].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '0.4rem 1rem', borderRadius: '6px', border: '1px solid #ddd', whiteSpace: 'nowrap', background: tab === t ? '#1a1a1a' : '#fff', color: tab === t ? '#fff' : '#333', cursor: 'pointer', textTransform: 'capitalize', fontWeight: tab === t ? 600 : 400, fontSize: '0.9rem', flexShrink: 0 }}>{t}</button>
        ))}
      </div>
